...
void setup() {
  ...
  Serial.println(">>> Keeping still for 3 seconds...                  <<<");

  // Calibration — 1500 samples at 2ms each = 3s
  uint32_t calSum = 0;
  for (int i = 0; i < 1500; i++) {
    ...
    if (i % 300 == 0) {
      ...
      Serial.print(i);
      Serial.print("  ADC=");
      Serial.println(analogRead(PIN_EOG));
    }
  }
  baseline = calSum / 1500;

  // Pre-fill MAF with baseline
  for (int i = 0; i < MAF_SIZE; i++) {
    mafBuf[i] = (uint16_t)baseline;
    mafSum += baseline;
  }
  mafReady = true;

  Serial.println("");
  Serial.printf(">>> Baseline: %d  (good = 1600–2400)\n", (int)baseline);
  if (baseline < 1400 || baseline > 2700)
    Serial.println("!!! Baseline out of range — check electrodes & ADC_11db");
  Serial.println("");
  Serial.println(">>> READY — Look left/right and blink <<<");
